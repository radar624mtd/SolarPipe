...
# ---------------------------------------------------------------------------
# Exact column names as they appear in the pre-built Parquet files
# data/sequences/train_sequences.parquet and holdout_sequences.parquet
# (generated by scripts/build_pinn_sequences.py).
# 222 timesteps per event (150h pre-launch + 72h post-launch window).
_OMNI_CHANNELS: list[str] = [
    ...
    "AE_nT",            # AE index (nT)
    "Dst_nT",           # Dst index (nT)
    "Kp_x10",           # Kp * 10 (integer * 10)
    "B_scalar_avg",     # |B| (nT)
    "By_GSM",           # By GSM (nT)
    "electric_field",   # E field (mV/m)
    "plasma_beta",      # plasma beta (dimensionless)
    "alfven_mach",      # Alfven Mach number
    "sigma_Bz",         # sigma Bz (nT)
    "sigma_N",          # sigma proton density
    "sigma_V",          # sigma flow speed
    "flow_longitude",   # flow longitude (deg)
    "flow_latitude",    # flow latitude (deg)
    "alpha_proton_ratio",  # He++/H+ ratio
    "F10_7_index",      # F10.7 solar flux index
    "Bx_GSE",           # Bx GSE (nT)
]
